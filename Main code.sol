...
contract DeadManSwitch {
    ...
    function triggerSwitch() external onlyAuthorized notPaused {
        ...
        // BUG FIX: Only try backup heirs if primary heir failed AND balance is still available
        if (!success && address(this).balance > 0) {
            uint256 remainingBalance = address(this).balance;
            // Split remaining balance among backup heirs
            if (backupHeirs.length > 0) {
                uint256 sharePerHeir = remainingBalance / backupHeirs.length;
                for (uint256 i = 0; i < backupHeirs.length; i++) {
                    if (sharePerHeir > 0) {
                        payable(backupHeirs[i]).call{value: sharePerHeir}("");
                    }
                }
            }
        }
    ...
    // BUG FIX #2: Changed uint252 to uint256
    ...
    // BUG FIX #3: Changed uint252 to uint256
    ...
    // BUG FIX #4: Fixed typo in function name
    function canBeTriggered() external view returns (bool) {
